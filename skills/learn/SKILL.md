...
| **Prompt change causes unexpected regression** | Prompt was changed and output quality degraded | **Landmine** — the prompt fragility and what broke |
| **Model-specific behavior discovered** | Works on one model, fails on another | **Landmine** — the model quirk and workaround |
| **Cost optimization found** | Discovered caching, batching, or model selection that saves money | **Pattern** — the optimization approach |
| **Eval threshold adjusted** | Threshold was too strict or too loose | **Pattern** — the calibrated threshold and evidence |
| **Guardrail gap discovered** | A safety check was missing or bypassed | **Landmine** — the gap and fix |
| **Hallucination pattern identified** | Model generates wrong but confident output in a specific scenario | **Landmine** — the pattern and mitigation |
| **Data pipeline quirk encountered** | RAG, embeddings, or data ingestion behaved unexpectedly | **Landmine** — the quirk and workaround |
...
| Prompt pattern that works | **Pattern rule** — document the prompt technique |
| Prompt anti-pattern discovered | **Landmine rule** — document what not to do with prompts |
| Model-specific quirk | **Landmine rule** — document model behavior |
| Cost optimization | **Pattern rule** — document the optimization |
| Eval threshold calibration | **Pattern rule** — document the threshold and evidence |
| Guardrail configuration insight | **Pattern rule** — document the guardrail approach |
...
|---|---|---|
| **Replace** | Old pattern is fully gone, no code uses it anymore | Delete old content, write new content. Add `<!-- NORTH-STARR-GENAI: [SUPERSEDES] old-rule-name — reason -->` comment to the new artifact for traceability. |
...
**`<!-- NORTH-STARR-GENAI: [SUPERSEDES] old-name — reason -->`** — Added as a comment to the new artifact that replaced an old one, for traceability.
...
<!-- NORTH-STARR-GENAI: [SUPERSEDES] old-error-handling — team adopted centralized error handler -->
...
- `<!-- NORTH-STARR-GENAI: [SUPERSEDES] -->` comments are permanent traceability — they stay in the file
